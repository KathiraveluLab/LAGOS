...
-spec be_ok({ok, FJP} | {error, any()}) -> FJP.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FJU}) -> FJU.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FJX)) -> FJX.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
