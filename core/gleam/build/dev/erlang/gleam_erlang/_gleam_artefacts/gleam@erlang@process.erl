...
-opaque subject(DLO) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DLO)}.

-type name(DLP) :: any() | {gleam_phantom, DLP}.

-type do_not_leak() :: any().

-type selector(DLQ) :: any() | {gleam_phantom, DLQ}.
...
-spec 'receive'(subject(DMO), integer()) -> {ok, DMO} | {error, nil}.
...
-spec receive_forever(subject(DMW)) -> DMW.
...
-spec selector_receive(selector(DNA), integer()) -> {ok, DNA} | {error, nil}.
...
-spec selector_receive_forever(selector(DNE)) -> DNE.
...
-spec map_selector(selector(DNG), fun((DNG) -> DNI)) -> selector(DNI).
map_selector(A, B) ->
...
-spec merge_selector(selector(DNK), selector(DNK)) -> selector(DNK).
merge_selector(A, B) ->
...
-spec select_map(selector(DNV), subject(DNX), fun((DNX) -> DNV)) -> selector(DNV).
...
-spec select(selector(DNR), subject(DNR)) -> selector(DNR).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DOF)
) -> selector(DOF).
...
-spec select_other(selector(DOJ), fun((gleam@dynamic:dynamic_()) -> DOJ)) -> selector(DOJ).
...
-spec deselect(selector(DOA), subject(any())) -> selector(DOA).
...
-spec select_specific_monitor(selector(DOV), monitor(), fun((down()) -> DOV)) -> selector(DOV).
...
-spec select_monitors(selector(DOY), fun((down()) -> DOY)) -> selector(DOY).
...
-spec select_trapped_exits(selector(DNO), fun((exit_message()) -> DNO)) -> selector(DNO).
...
-spec deselect_specific_monitor(selector(DPB), monitor()) -> selector(DPB).
...
-spec send_after(subject(DPX), integer(), DPX) -> timer().
...
-spec perform_call(
    subject(DPE),
    fun((subject(DPG)) -> DPE),
    fun((selector(DPG)) -> {ok, DPG} | {error, nil})
) -> DPG.
...
-spec call(subject(DPL), integer(), fun((subject(DPN)) -> DPL)) -> DPN.
...
-spec call_forever(subject(DPP), fun((subject(DPR)) -> DPP)) -> DPR.
