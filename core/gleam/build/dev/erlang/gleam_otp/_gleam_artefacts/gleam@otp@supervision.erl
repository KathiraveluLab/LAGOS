...
-type child_specification(EOE) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(EOE)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(EOF)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EOF).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(EOK)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(EOK).
supervisor(Start) ->
...
-spec significant(child_specification(EOP), boolean()) -> child_specification(EOP).
...
-spec timeout(child_specification(EOS), integer()) -> child_specification(EOS).
timeout(Child, Ms) ->
...
-spec restart(child_specification(EOV), restart()) -> child_specification(EOV).
...
-spec map_data(child_specification(EOY), fun((EOY) -> EPA)) -> child_specification(EPA).
