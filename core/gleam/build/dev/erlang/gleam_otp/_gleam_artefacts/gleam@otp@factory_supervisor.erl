...
-opaque supervisor(EQS, EQT) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(EQS, EQT))}.

-type message(EQU, EQV) :: any() | {gleam_phantom, EQU, EQV}.

-opaque builder(EQW, EQX) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((EQW) -> {ok, gleam@otp@actor:started(EQX)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(EQW, EQX)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(EQY, EQZ) :: {local,
        gleam@erlang@process:name(message(EQY, EQZ))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(ERA) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, ERA}.
...
-type erlang_child_spec_property(ERB, ERC) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((ERB) -> {ok, gleam@otp@actor:started(ERC)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec start(builder(ESV, ESW)) -> {ok,
        gleam@otp@actor:started(supervisor(ESV, ESW))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec start_child(supervisor(EUB, EUC), EUB) -> {ok,
        gleam@otp@actor:started(EUC)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((EUZ) -> {ok, gleam@otp@actor:started(EVA)} |
        {error, gleam@otp@actor:start_error()}),
    EUZ
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), EVA, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((ERK) -> {ok, gleam@otp@actor:started(ERL)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(ERK, ERL).
...
-spec supervisor_child(
    fun((ERP) -> {ok, gleam@otp@actor:started(ERQ)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(ERP, ERQ).
