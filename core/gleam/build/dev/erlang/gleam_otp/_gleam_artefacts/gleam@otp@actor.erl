...
-type message(DUZ) :: {message, DUZ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(DVA, DVB) :: {continue,
        DVA,
        gleam@option:option(gleam@erlang@process:selector(DVB))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(DVC, DVD) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        DVC,
        gleam@erlang@process:selector(message(DVD)),
        gleam@otp@system:debug_state(),
        fun((DVC, DVD) -> next(DVC, DVD))}.

-type started(DVE) :: {started, gleam@erlang@process:pid_(), DVE}.

-opaque initialised(DVF, DVG, DVH) :: {initialised,
        DVF,
        gleam@option:option(gleam@erlang@process:selector(DVG)),
        DVH}.

-opaque builder(DVI, DVJ, DVK) :: {builder,
        fun((gleam@erlang@process:subject(DVJ)) -> {ok,
                initialised(DVI, DVJ, DVK)} |
            {error, binary()}),
        integer(),
        fun((DVI, DVJ) -> next(DVI, DVJ)),
        gleam@option:option(gleam@erlang@process:name(DVJ))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(DVL) :: {ack, {ok, DVL} | {error, binary()}} |
...
-spec continue(DVQ) -> next(DVQ, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(DWC, DWD), gleam@erlang@process:selector(DWD)) -> next(DWC, DWD).
...
-spec initialised(DWJ) -> initialised(DWJ, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(DWO, any(), DWQ),
    gleam@erlang@process:selector(DWU)
) -> initialised(DWO, DWU, DWQ).
...
-spec returning(initialised(DWZ, DXA, any()), DXF) -> initialised(DWZ, DXA, DXF).
...
-spec new(DXJ) -> builder(DXJ, DXK, gleam@erlang@process:subject(DXK)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(DXP)) -> {ok, initialised(DXR, DXP, DXS)} |
        {error, binary()})
) -> builder(DXR, DXP, DXS).
...
-spec on_message(builder(DYB, DYC, DYD), fun((DYB, DYC) -> next(DYB, DYC))) -> builder(DYB, DYC, DYD).
...
-spec named(builder(DYM, DYN, DYO), gleam@erlang@process:name(DYN)) -> builder(DYM, DYN, DYO).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(DZB))) -> gleam@erlang@process:selector(message(DZB)).
...
-spec receive_message(self(any(), DYX)) -> message(DYX).
...
-spec initialise_actor(
    builder(any(), any(), DZU),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, DZU} | {error, binary()})
...
-spec start(builder(any(), any(), EAH)) -> {ok, started(EAH)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EAO), EAO) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EAQ),
    integer(),
    fun((gleam@erlang@process:subject(EAS)) -> EAQ)
) -> EAS.
